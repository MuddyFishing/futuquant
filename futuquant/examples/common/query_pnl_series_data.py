...
def get_pnl_series_data(quote_context, code, start='2017-01-01', end='2017-12-30', code_base=None):
    ...
    :return: (ret, data) ret == 0 data为pd dataframe数据， 表头'code' 'DateTime' 'pnl' 'pnl_supper'(需指定code_base)
                         ret != 0 data为错误字符串
    '''
    # raw pnl
    raw_dst = None
    ...
    for ix, row in trade_data.iterrows():
        ...
        if raw_base is not None:
            ...
            dict_item['pnl_supper'] = last_pnl - last_base_pnl

        list_ret.append(dict_item.copy())

    col_list = ['code', 'DateTime', 'pnl']
    if raw_base is not None:
        col_list.append('pnl_supper')
