...
class TestAll(object):
    ...
    def test_quote(self,logger):
        ...
        logger.info(quote_ctx.get_autype_list(code_list=['HK.00388', 'US.AAPL', 'SZ.300104']))
        logger.info('获取市场快照 get_market_snapshot')
        logger.info(quote_ctx.get_market_snapshot(code_list=['HK.00388', 'US.AAPL', 'SZ.300104']))
        ...
        logger.info(
            quote_ctx.get_history_kline(code='HK.00388', start=None, end=None, ktype=KLType.K_DAY, autype=AuType.QFQ,
                                        fields=[KL_FIELD.ALL]))
        logger.info(
            quote_ctx.get_history_kline(code='US.AAPL', start=None, end=None, ktype=KLType.K_MON, autype=AuType.HFQ,
                                        fields=[KL_FIELD.ALL]))
        logger.info(
            quote_ctx.get_history_kline(code='SZ.300601', start=None, end=None, ktype=KLType.K_WEEK, autype=AuType.NONE,
                                        fields=[KL_FIELD.ALL]))
        logger.info('获取多支股票多个单点历史K线 get_multi_points_history_kline')
        logger.info(quote_ctx.get_multi_points_history_kline(code_list=['HK.00388', 'US.JD', 'SH.000001'],
        ...
        logger.info(quote_ctx.get_cur_kline(code='HK.00388', num=1000, ktype=SubType.K_5M, autype=AuType.QFQ))
    ...
    def test_trade(self,logger,tradeEnv):
        ...
        logger.info(trade_hk.acctradinginfo_query(order_type=OrderType.NORMAL, code=code_hk, price=price_hk, order_id=0,
        ...
        logger.info(
            trade_hk.modify_order(modify_order_op=ModifyOrderOp.NORMAL, order_id=order_ids_hk[0], qty=qty_hk * 2,
                                  price=price_hk - 1, adjust_limit=0,
                                  trd_env=tradeEnv, acc_id=0))

        logger.info('美股改单，order_id = ' + order_ids_us[0])
        logger.info(
            trade_us.modify_order(modify_order_op=ModifyOrderOp.NORMAL, order_id=order_ids_us[0], qty=qty_us * 2,
        ...
        logger.info(
            trade_cn.modify_order(modify_order_op=ModifyOrderOp.NORMAL, order_id= order_ids_cn[0], qty=qty_cn * 2,
        ...
        logger.info('HK 持仓列表')
        logger.info(
            trade_hk.position_list_query(code='', pl_ratio_min=None, pl_ratio_max=None, trd_env=tradeEnv, acc_id=0))
        logger.info('US 持仓列表')
        logger.info(
            trade_us.position_list_query(code='', pl_ratio_min=None, pl_ratio_max=None, trd_env=tradeEnv, acc_id=0))
        logger.info('CN 持仓列表')
        logger.info(
            trade_cn.position_list_query(code='', pl_ratio_min=None, pl_ratio_max=None, trd_env=tradeEnv, acc_id=0))
    ...
    def test_trade_async(self,logger,tradeEnv):
